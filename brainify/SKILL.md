...
2. **inspect** — 항목별 `brainify.py inspect "<item>"`. 스레드 본문(`_thread.md`)과
   ...
   - **★ xlsx 는 파싱한다 (2026-08-07 개정 — 구 "생략" 정책 폐기)**: 2026-06-07 엔 "명단·시트류는 구조화 데이터 자체가 값이라 full-text 파싱 가치 낮음" 으로 `_parse` 생략을 정했으나, **실측이 반대였다** — vault xlsx 52개 중 50개가 이미 파싱돼 `refined.md` 가 있었고 **노트 51개가 그 풀텍스트를 근거로 쓰였다**. 게다가 `_parse()` 는 `_refined()` 를 먼저 보므로 refined.md 가 있으면 `skipped-xlsx` 분기에 **도달조차 하지 않아**, 정책이 사실상 죽어 있었다. 비용도 근거가 못 된다(xlsx docling 은 수 초 — 방대 PDF 의 30분과 다르다).
     ...
     - `refined.md` 가 아직 없을 때만 `via: skipped-xlsx` 가 나온다 — 이제 그건 *정책*이 아니라 **아직 파싱 전**이라는 뜻이다. 급하면 경량 추출(`zipfile` → `sharedStrings.xml`)로 즉시 파악한다(Read 도구는 xlsx 렌더 못 함). **방금 도착한 파일(2026-09-18)은 `skipped-xlsx` 대신 `via: pending-drain`** — PDF 와 동일한 grace 창(§via 표 참조), 헤드리스는 이 항목을 skip(§헤드리스).
   ...
   - **★ 부서 내부 구성 (한 사안 = 한 정본 + 한 소스폴더 — 2026-06-07)**: `<부서>` 로 라우팅한 *뒤*, 그 안을 이렇게 구성한다.
     ...
     - **표준 레이아웃**: `knowledge/02_areas/<조직>/<부서>/<사안식별자>.md`(정본, 부서 최상위 flat) ↔
       ...
       - **★ 사안 명명 규약 (2026-06-07)**: `YYYY-MM-DD_<조직토큰>_<부서>_<사안>`. 구조 구분자 = **`_` 통일**(날짜·조직·부서·사안 경계), 사안 내부는 무구분(예 `27대2차회의`). 사안 예: `27대N차회의`·`27대명단`·`27대N차일정`(씨앗).
         **전역 자기서술 유지** — 조직·부서 토큰을 *원칙적으로 포함*(폴더 경로와 중복돼도): wikilink basename 은 폴더 맥락 없이 전역 고유·인지돼야 함(링크·인박스가 부서 밖에서 참조). 이 중복은 *전역 주소성의 대가*.
         - **예외 — 사안명 자체가 고유명사면 조직 토큰 생략 (2026-09-13 신설)**: 법정계획·법령·고시·간행물처럼 **문서명이 이미 전역 고유**하면 토큰이 주소성을 더하지 않고 글자만 늘린다. 판별 = **"노트명만 보고 무엇인지 알겠는가?"**
           - 생략 ○ `2010-03_제1차원자력안전종합계획`(원안위만 만드는 법정계획) · `2015-03_제1차국가방사능방재계획`
           - 생략 ✗ `2024-08-27_KARP_의학위원회_2024년3차회의`(토큰 없으면 무슨 회의인지 불명 — **토큰이 의미를 지탱**)
           - 즉 규칙의 *목적*(전역 자기서술)은 그대로고, 토큰은 그 목적을 달성하는 *수단일 뿐*이다. 목적이 이미 충족되면 수단은 생략한다.
           - 실측 근거: 2026-09-13 `02_areas/조직/원자력안전위원회/` 법정계획 6건에서 `_원안위_` 제거(Dr. Ben 지시 — 폴더가 이미 원안위인데 파일마다 반복돼 인지부하). frontmatter `tags` 에 조직명이 남아 검색성 유지.
