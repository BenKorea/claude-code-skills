...
| 동반 노트 표준 | CLAUDE.md 의 frontmatter (sources 상대경로 + **parse 상대경로**) + 본문 첫 줄 `[원본 PDF](sources/...)` + 요약·내 생각·관련 노트 링크 | 확정 (parse 필드 2026-05-15 추가) |
| 영속 상태 | (필요 시) `~/.claude/skills/brainify-inbox/state/processed.jsonl` — 처리 이력 (SHA·원본 경로·정착 경로·`parse_dir`·diff 채택 사유). git 미추적 | **P3.4 에서 skip 결정** (Notes 참조) |
| **파싱 원본 보존 (2026-05-15, 폴더명 2026-05-19 개정)** | `sources/<PARA>/<원본파일명.ext>_parse/` — 원본 파일 바로 옆 폴더 (원본파일명 전체+확장자에 `_parse` 접미, 산출물 직접 수납. **구 `<stem>/_parse/` 의 빈 중간 폴더 제거**). 3종 산출물: ① raw JSON 3개 (`docling.json` / `mineru.json` / `diff.json`) — entrypoint stdout 그대로. ② Obsidian 비교용 markdown 2개 (`docling.md` / `mineru.md`) — JSON 의 markdown 필드 추출. ③ **정제본 (`refined.md`)** — Claude 가 raw 둘을 보정한 풀텍스트 (§3 에서 작성, frontmatter + 본문). MinerU sub-artifacts (layout.pdf·span.pdf·images/) 는 tempdir cleanup 손실 → Phase 2. 도출 규칙 = `sources:` + `_parse` 결합 (결정론적·확장자 포함이라 동일 stem 다중 확장자 충돌 안전). 적용: **신규부터, 기존 5건은 점진 전환** | 확정 |
| **자료 위계 (2026-05-15)** | PDF 원본 ↔ raw 파싱 (docling/mineru) ↔ **정제본 (refined.md, 풀텍스트)** ↔ 동반 노트 (요약·관찰·연결). refined 와 동반 노트의 역할 분리 — refined 는 원본 충실, 동반 노트는 인지·메타 | 확정 |
...
- 2026-05-15 — **파싱 원본 보존 도입** (Dr. Ben 피드백: "파싱원본이 있었을 것이다 — 현재는 요약만 존재"). §Plan 의 "동반 노트 표준" 행에 `parse:` frontmatter 필드 추가, "파싱 원본 보존" 행 신설. SKILL.md §7-2 에 `sources/<PARA>/<새stem>/_parse/{docling,mineru,diff}.json` sub-folder 생성 단계 추가, §8 에 WORKDIR rmdir + `parse_dir` 필드 추가. **MinerU sub-artifacts (layout.pdf·span.pdf·images/)** 는 entrypoint `parse_mineru` 의 tempdir cleanup 에서 손실 (현재 markdown + middle.json 만 dict packaging) — Phase 2 future work. **기존 5건 노트북 백채움**: 한국원자력의학원 수입지출현황·임직원수 + 대한핵의학회 학회확인증 3건. 분업 규칙 일시 우회 (노트북에서 SKILL.md / PROGRESS.md 직접 수정) — Dr. Ben 양 PC 단일 사용자라 동시 작업 risk 0 보장 + 데스크탑 sb-claude 메모리 해제 완료 (compose `restart: unless-stopped` 한 줄 + systemd disable 도 함께 진행).
- 2026-05-15 (오후) — **정제본 (refined.md) 도입 + 백채움 완료** (Dr. Ben 피드백: "두 차이 보정한 최종 정제본도 저장하자 — 정제된 좋은 자료"). §Plan 의 "파싱 원본 보존" 행에 정제본 추가, "자료 위계" 행 신설 (PDF ↔ raw ↔ refined ↔ 동반노트). SKILL.md §3 에 정제본 작성 단계 추가 (frontmatter 표준: source_pdf·base_engine·corrections·generated·host + 본문 풀텍스트). 역할 분리 — 동반 노트는 요약·관찰·메타, refined.md 는 원본 충실 재현 풀텍스트. **기존 5건 백채움 완료** — refined.md 5건이 06:30~06:39 노트북에서 작성 후 vault Syncthing 으로 데스크탑 반영(06:42 commit `90b2116` 시점에 이미 완료, 메모만 stale 한 채 push 되어 데스크탑에서 정정). 메모리 [[cross-check-vault-before-git-memo]] 신규 — git 동기 자산(PROGRESS.md 등)의 진행 메모와 Syncthing 동기 자산(vault `_parse/`)의 실측 사이 갭 차단.
- 2026-05-19 — **파싱 폴더명 규약 개정** (Dr. Ben 결정, PARA 재검토 세션 중). `<새stem>/_parse/` (빈 중간 폴더 + 중첩) → `<원본파일명.ext>_parse/` (단일 폴더, 확장자 포함 접미). 근거: 중복 레이어 제거 + 짝 관계가 접미사로 명시 + 도출 규칙이 `sources:`+`_parse` 결합으로 무판단·결정론적·충돌 안전. §Plan "파싱 원본 보존" 행 + SKILL.md §7-2 (PARSE_DIR·frontmatter `parse:`) 갱신. vault CLAUDE.md 「파일명 규칙」에 동반 파생 폴더 표준 1줄 추가. **적용: 신규부터. 기존 5건(`<stem>/_parse/`) 점진 전환** (그 자료 만질 때 — 프로젝트 폴더 규칙·frontmatter 마이그레이션과 동일 철학). 점진 전환 시 동반 노트 `parse:` frontmatter 동시 갱신 필수.
- 작성자: Dr. Ben + Claude.
