...
| **파싱 원본 보존 (2026-05-15)** | `sources/<PARA>/<새stem>/_parse/{docling,mineru,diff}.json` — 원본 PDF 와 같은 디렉토리의 `_parse/` sub-folder. SKILL.md §7-2 단계. 3개 JSON 은 entrypoint stdout 그대로 보존. MinerU sub-artifacts (layout.pdf·span.pdf·images/) 는 entrypoint 의 tempdir cleanup 에서 손실 → Phase 2 future work. `_` 접두어로 Obsidian 정렬·검색 잡음 최소화 | 확정 |
