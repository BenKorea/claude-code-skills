...
| **파싱 원본 보존 (2026-05-15)** | `sources/<PARA>/<새stem>/_parse/` — 원본 PDF 와 같은 디렉토리의 sub-folder. 3종 산출물: ① raw JSON 3개 (`docling.json` / `mineru.json` / `diff.json`) — entrypoint stdout 그대로. ② Obsidian 비교용 markdown 2개 (`docling.md` / `mineru.md`) — JSON 의 markdown 필드 추출. ③ **정제본 (`refined.md`)** — Claude 가 raw 둘을 보정한 풀텍스트 (§3 에서 작성, frontmatter + 본문). MinerU sub-artifacts (layout.pdf·span.pdf·images/) 는 tempdir cleanup 손실 → Phase 2. `_` 접두어로 Obsidian 정렬 잡음 최소화 | 확정 |
